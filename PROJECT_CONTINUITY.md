...
- **Releases are now fully automatic on every push to `main`**: `release.yml` triggers on `push: branches [main]` (+ `workflow_dispatch`, tags kept as an extra), auto-computes `v1.0.N+1` from the latest release/tag, builds the APK/AAB and publishes a new versioned release (name `v1.0.N`, tag `v1.0.N`, assets `turna.apk`/`turna.aab`/`version.json` + bundle/checksums) with `gh release create/.upload`, mirrors `turna.apk`/`turna.aab`/`version.json` to the downloads repo (rolling `latest` release so `/api/app/version` stays bounded), then deploys Vercel. `release.yml` is the single Android build per push — `ci.yml` now runs typecheck/lint/tests only (`permissions: contents: read`), and `concurrency: cancel-in-progress` on both workflows keeps only the newest push building.
...
- **Solo Ledger rotation window**: create popup now takes **Start month + End month** (end clamped ≥ start, default current → current+6) and derives the name only — `deriveLedgerName` → `"June – December 2026"` (`formatPeriodLabel(start) + ' – ' + formatPeriodLabel(end)`), parsed back with `parsePeriodRange(name): {start,end} | null` (legacy single-month names still work via `parseStartPeriod`; no DB columns — window lives in `name`). Sheet: № column (type a turn number → rows re-sort, `sort_order` renormalised 0..n-1; add-row takes the next free number), rotation anchored at the window's start with `((monthsBetween(start,P)) mod rowCount)`, nav/swipe/auto-advance clamped to [start,end] ("Out of cycle"/"Cycle completed" outside), `Collector gets ₦X` + `₦5,000 × 8 people = ₦40,000 pot` stats line, header shows `8 months · 3 collectors`.
